<script type="text/javascript">
	...
	<?php if($u_nivel==2): ?>
		//MESES DEL MUNICIPIO DEL OPERADOR
		$.ajax({
			url: 'sql_charts.php',
			type: 'post',
			data: {municipio_m: <?php echo $u_municipio ?>},
		})
		.done(function(lista) {
			var mes=JSON.parse(lista);
			mes_total(mes);
		...
		//DIAS DEL MUNICIPIO DEL OPERADOR
		$.ajax({
			url: 'sql_charts.php',
			type: 'post',
			data: {municipio_d: <?php echo $u_municipio ?>},
		})
		.done(function(lista) {
			var dia=JSON.parse(lista);
			dia_total(dia);
